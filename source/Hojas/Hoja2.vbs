...
Public Sub PlantillaAprobacion()
    Hoja2.Unprotect pword
    Hoja2.Range("APROB_SOL") = ""
    Hoja2.Range("APROB_RENOV") = ""
    Hoja2.Range("APROB_NOMBRE") = ""
    Hoja2.Range("APROB_CSOCIO") = ""
    Hoja2.Range("APROB_PREST") = ""
    Hoja2.Range("APROB_MON") = ""
    Hoja2.Range("APROB_MONTO") = ""
    Hoja2.Range("APROB_PLAZO") = ""
    Hoja2.Range("APROB_TEA") = ""
    Hoja2.Range("APROB_GARANTIA") = ""
    Hoja2.Range("APROB_NPLAZO") = ""
    Hoja2.Range("APROB_PLAZOT") = ""
    Hoja2.Protect Password:=pword, DrawingObjects:=True, Contents:=True, Scenarios:=True
End Sub
...
Public Sub LlenarAprobacion()
    ...
    Hoja2.Unprotect pword
    Hoja2.Range("APROB_SOL") = Hoja3.tbNSol.Text
    If Len(Hoja3.tbNRenov.Text) <= 2 Then
        Hoja2.Range("APROB_RENOV") = Left("00", 2 - Len(Hoja3.tbNRenov.Text)) & Hoja3.tbNRenov.Text
    Else
        Hoja2.Range("APROB_RENOV") = Hoja3.tbNRenov.Text
    End If
    
    Hoja2.Range("APROB_NOMBRE") = Hoja3.tbNomSocio.Text
    
    If Len(Hoja3.tbCSocio.Text) <= 7 Then
        Hoja2.Range("APROB_CSOCIO") = Left("0000000", 7 - Len(Hoja3.tbCSocio.Text)) & Hoja3.tbCSocio.Text
    Else
        Hoja2.Range("APROB_CSOCIO") = Hoja3.tbCSocio.Text
    End If
    
    Hoja2.Range("APROB_PREST") = Hoja3.tbCodP.Text
    
    If Hoja3.obSoles Then
        Hoja2.Range("APROB_MON") = "SOLES"
        Hoja2.Range("APROB_MONTO") = "S/." & Right(Format(Hoja3.tbMonto.Value, "Currency"), Len(Format(Hoja3.tbMonto.Value, "Currency")) - 3)
    Else
        Hoja2.Range("APROB_MON") = "DOLARES"
        Hoja2.Range("APROB_MONTO") = "US$" & Right(Format(Hoja3.tbMonto.Value, "Currency"), Len(Format(Hoja3.tbMonto.Value, "Currency")) - 3)
    End If
    
    Hoja2.Range("APROB_PLAZO") = Hoja3.Range("PLAZO").Value & " d僘s"
    
    Hoja2.Range("APROB_TEA") = Hoja3.tbTEA.Text & "%"
    
    Hoja2.Range("APROB_GARANTIA") = Hoja3.tbGarantias.Text
    
    Hoja2.Range("APROB_NPLAZO") = Int(Hoja3.Range("PLAZO_NUEVO").Value) - Int(Hoja3.Range("PLAZO").Value) & " d僘s"
    
    Hoja2.Range("APROB_PLAZOT") = Int(Hoja3.Range("PLAZO_NUEVO").Value) & " d僘s"
        closeRS
        OpenDB
